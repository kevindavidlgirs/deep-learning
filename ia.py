class IA:
    ...
    def __back_propagation(self,responses):
        ...
        for i in reversed(range(0, len(self.__neurons)-1)):
            h_l_error = np.dot(delta[len(self.__neurons)-2-i], self.__weights[i].T)
            delta.append(h_l_error*tools.sigmoidPrime(self.__neurons[i]))
